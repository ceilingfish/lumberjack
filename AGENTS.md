...
- **Lifecycle is managed via [`github.com/kardianos/service`](https://github.com/kardianos/service).** The `daemon` command is a parent with five subcommands (one file each under `cmd/`, `daemon_*.go`):
  - `daemon run` — run in the foreground; blocks until signalled. This is also the entry point the platform service manager launches (`service.Run` adapts to the foreground vs service-managed context).
  - `daemon install` — register the daemon to start automatically. Installed as a **per-user agent** (a LaunchAgent under `~/Library/LaunchAgents` on macOS) so it runs as the invoking user, keeping `~/.lumberjack` paths and the user's `gh auth` credentials valid. No sudo. Starts at login (`RunAtLoad`) and is restarted if it exits (`KeepAlive`). macOS is the only tested platform for now; other OSes come from the library later.
  - `daemon start` / `daemon stop` — delegate to the service manager (`svc.Status()` then `svc.Start()`/`svc.Stop()`); both no-op with a clear message if already in the target state, and report "not installed — run `lumberjack daemon install` first" when the agent is absent.
  - `daemon status` — reports installed/running/stopped via the manager, augmented with the live PID read from `~/.lumberjack/daemon.pid`, which the daemon writes on startup and removes on shutdown (a fx lifecycle hook in `internal/daemon`). Exits non-zero when not installed.
