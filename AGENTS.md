...
- **`proto/` is the source of truth.** Change the API by editing the `.proto` files and regenerating; never hand-edit generated code. Run `buf lint` and `buf breaking` before committing changes to the service. Generated stubs are committed so a plain `go build` needs no protoc toolchain. CI always runs `buf breaking` against `main`, and a wire break fails the build unless a commit on the branch declares a breaking change the way release-please expects — a `!` after the type/scope, or a `BREAKING CHANGE:` footer. Declaring one does not hide the break: every finding is still reported, as a warning annotation and in the job summary. That is how a deliberate break (removing a field or an RPC) gets merged while staying visible in the changelog and the version bump. Both components ship as one binary, but the macOS app versions separately and can talk to a newer daemon, so check what a removal costs an already-released app before declaring the break.
...
- **Non-trivial task logic lives in `scripts/`, not inline in `.mise.toml`.** `mise` tasks should stay one-liners that call a script in `scripts/` (e.g. `scripts/coverage-gate.ps1`).
- **Per-codebase tasks are prefixed `cli:` or `osx:`.** `cli:format`, `cli:lint` and `cli:test` act on the Go CLI/daemon; `osx:format`, `osx:lint` and `osx:test` act on the Swift menu-bar app under `macos/`. The two share no task — the toolchains, file sets and failure modes have nothing in common, and a single combined task only ever means "run something irrelevant to what I changed". **The one exception is `coverage`**, which is deliberately shared: it runs no tests itself, but depends on `cli:test` and `osx:test` and gates the coverage data they produce. Add a new per-codebase task under the matching prefix rather than growing an unprefixed one.
...
- **Commits must use [Conventional Commits](https://www.conventionalcommits.org/).** release-please parses commit subjects to compute each component's next version (see issue #11) — a non-conventional subject produces no release. Use `feat:` for a minor bump, `fix:` for a patch, and either a `!` after the type/scope (e.g. `feat!:`) or a `BREAKING CHANGE:` footer for a breaking change (both components are pre-1.0, so a breaking change bumps the minor, not the major, until either cuts a `v1.0.0`). Other conventional types (`chore:`, `docs:`, `refactor:`, `test:`, etc.) are accepted but don't trigger a release on their own. **The paths a commit touches decide which component it versions:** anything under the macOS project directory (`macos/`) bumps the app, everything else bumps the Go CLI, and a commit touching both bumps both — there is no way to opt a commit out of this via its type or scope.
- **Never write code comments** Comments result in the codebase bloating, and your comments are _way_ too verbose. If you think something needs documenting, beg the author to add them manually. You _never_ add comments automatically.
