...
func dispatchRepository(ctx context.Context, cmd *cobra.Command, cl *client.Client, args []string, force bool, format present.Format) error {
	...
	case len(rest) == 0:
		...
		if repo.GetSetupConsentPending() {
			return promptSetupConsent(ctx, cmd, cl, name)
		}
		return nil
...
// promptSetupConsent checks whether the repository resolved by name has
...
func promptSetupConsent(ctx context.Context, cmd *cobra.Command, cl *client.Client, name string) error {
	pending, commands, err := cl.GetSetupConsent(ctx, name)
	if err != nil {
		return err
	}
	if !pending {
	...
	if !confirm(cmd, "Allow Lumberjack to run these commands in new worktrees for this repository?") {
		_, err := fmt.Fprintln(out, "Not consented — the daemon will skip these commands until you run `lumberjack repositories NAME` and consent.")
		return err
	}
	if _, err := cl.SetSetupConsent(ctx, name); err != nil {
		return err
	}
