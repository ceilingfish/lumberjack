	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return withClient(cmd, func(ctx context.Context, cl *client.Client) error {
				return deleteWorktree(ctx, cmd, cl, ref, args[0], force, format)
			})
		},
	}
...
func deleteWorktree(ctx context.Context, cmd *cobra.Command, cl *client.Client, ref, worktree string, force bool, format present.Format) error {
	out := cmd.OutOrStdout()
