...
A sync also drops the **ghosts**: tracked worktrees that have neither a pull request nor a worktree left behind them, because the directory was deleted or `git worktree remove`d by hand. A worktree with no PR is otherwise never cleaned up automatically — without a finished PR there is nothing to say its work is safe to discard — so a ghost would stay listed forever. Only the bookkeeping is dropped; nothing is removed from disk, so a husk of ignored build artifacts left behind by a removal is the user's to keep. When `git worktree list` itself fails, the sync prunes only worktrees whose directory is genuinely gone: a failed listing is not evidence that git has forgotten a worktree.
