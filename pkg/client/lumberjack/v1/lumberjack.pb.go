// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        (unknown)
// source: lumberjack/v1/lumberjack.proto

// Package lumberjack.v1 is the gRPC contract between the Lumberjack CLI
// (client) and the background daemon (server). The daemon owns the database
// and all worktree operations; the CLI is a thin client that calls into this
// API. See docs/prd.md ("Architecture: client / server split") and AGENTS.md.
//
// `lumberjack doctor` deliberately has no RPC here: it checks host
// prerequisites (git/gh) and must work even when the daemon is not running, so
// it stays entirely CLI-local.

package lumberjackv1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// SyncStatus mirrors schema.SyncStatus* (see internal/database/schema).
type SyncStatus int32

const (
	SyncStatus_SYNC_STATUS_UNSPECIFIED SyncStatus = 0
	SyncStatus_SYNC_STATUS_OK          SyncStatus = 1
	SyncStatus_SYNC_STATUS_ERROR       SyncStatus = 2
)

// Enum value maps for SyncStatus.
var (
	SyncStatus_name = map[int32]string{
		0: "SYNC_STATUS_UNSPECIFIED",
		1: "SYNC_STATUS_OK",
		2: "SYNC_STATUS_ERROR",
	}
	SyncStatus_value = map[string]int32{
		"SYNC_STATUS_UNSPECIFIED": 0,
		"SYNC_STATUS_OK":          1,
		"SYNC_STATUS_ERROR":       2,
	}
)

func (x SyncStatus) Enum() *SyncStatus {
	p := new(SyncStatus)
	*p = x
	return p
}

func (x SyncStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SyncStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_lumberjack_v1_lumberjack_proto_enumTypes[0].Descriptor()
}

func (SyncStatus) Type() protoreflect.EnumType {
	return &file_lumberjack_v1_lumberjack_proto_enumTypes[0]
}

func (x SyncStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SyncStatus.Descriptor instead.
func (SyncStatus) EnumDescriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{0}
}

// WorktreeAction is what a sync (or init) did to a single branch's worktree,
// reported per branch so the CLI can render a branch/PR/action table.
type WorktreeAction int32

const (
	WorktreeAction_WORKTREE_ACTION_UNSPECIFIED WorktreeAction = 0
	// Created a new worktree for an open PR.
	WorktreeAction_WORKTREE_ACTION_CHECKED_OUT WorktreeAction = 1
	// Adopted a directory git already had checked out into tracking.
	WorktreeAction_WORKTREE_ACTION_ADOPTED WorktreeAction = 2
	// Linked an already-tracked worktree to its open PR.
	WorktreeAction_WORKTREE_ACTION_UPDATED WorktreeAction = 3
	// Removed a worktree whose PR has closed.
	WorktreeAction_WORKTREE_ACTION_DELETED WorktreeAction = 4
	// Kept a closed-PR worktree that still holds unmerged local work.
	WorktreeAction_WORKTREE_ACTION_RETAINED WorktreeAction = 5
)

// Enum value maps for WorktreeAction.
var (
	WorktreeAction_name = map[int32]string{
		0: "WORKTREE_ACTION_UNSPECIFIED",
		1: "WORKTREE_ACTION_CHECKED_OUT",
		2: "WORKTREE_ACTION_ADOPTED",
		3: "WORKTREE_ACTION_UPDATED",
		4: "WORKTREE_ACTION_DELETED",
		5: "WORKTREE_ACTION_RETAINED",
	}
	WorktreeAction_value = map[string]int32{
		"WORKTREE_ACTION_UNSPECIFIED": 0,
		"WORKTREE_ACTION_CHECKED_OUT": 1,
		"WORKTREE_ACTION_ADOPTED":     2,
		"WORKTREE_ACTION_UPDATED":     3,
		"WORKTREE_ACTION_DELETED":     4,
		"WORKTREE_ACTION_RETAINED":    5,
	}
)

func (x WorktreeAction) Enum() *WorktreeAction {
	p := new(WorktreeAction)
	*p = x
	return p
}

func (x WorktreeAction) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (WorktreeAction) Descriptor() protoreflect.EnumDescriptor {
	return file_lumberjack_v1_lumberjack_proto_enumTypes[1].Descriptor()
}

func (WorktreeAction) Type() protoreflect.EnumType {
	return &file_lumberjack_v1_lumberjack_proto_enumTypes[1]
}

func (x WorktreeAction) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use WorktreeAction.Descriptor instead.
func (WorktreeAction) EnumDescriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{1}
}

// LockStrategy is what Tidy does with a misplaced worktree git has locked.
// `git worktree move` refuses to move a locked worktree, so one of these must
// be chosen before the move can be attempted. The CLI resolves it from
// `--lock-strategy`, or by prompting per locked worktree.
type LockStrategy int32

const (
	// Unset. The daemon treats it as SKIP: it never prompts (only the CLI can),
	// and leaving a lock in place is the choice that changes nothing.
	LockStrategy_LOCK_STRATEGY_UNSPECIFIED LockStrategy = 0
	// Leave the worktree where it is, reporting it as skipped because it is
	// locked.
	LockStrategy_LOCK_STRATEGY_SKIP LockStrategy = 1
	// Unlock the worktree, move it, then lock it again with the same reason.
	LockStrategy_LOCK_STRATEGY_UNLOCK LockStrategy = 2
	// Unlock the worktree, move it, and leave it unlocked.
	LockStrategy_LOCK_STRATEGY_DELETE LockStrategy = 3
	// Cancel the whole tidy when any worktree in scope is locked: nothing is
	// moved and the daemon returns ABORTED.
	LockStrategy_LOCK_STRATEGY_ABORT LockStrategy = 4
)

// Enum value maps for LockStrategy.
var (
	LockStrategy_name = map[int32]string{
		0: "LOCK_STRATEGY_UNSPECIFIED",
		1: "LOCK_STRATEGY_SKIP",
		2: "LOCK_STRATEGY_UNLOCK",
		3: "LOCK_STRATEGY_DELETE",
		4: "LOCK_STRATEGY_ABORT",
	}
	LockStrategy_value = map[string]int32{
		"LOCK_STRATEGY_UNSPECIFIED": 0,
		"LOCK_STRATEGY_SKIP":        1,
		"LOCK_STRATEGY_UNLOCK":      2,
		"LOCK_STRATEGY_DELETE":      3,
		"LOCK_STRATEGY_ABORT":       4,
	}
)

func (x LockStrategy) Enum() *LockStrategy {
	p := new(LockStrategy)
	*p = x
	return p
}

func (x LockStrategy) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (LockStrategy) Descriptor() protoreflect.EnumDescriptor {
	return file_lumberjack_v1_lumberjack_proto_enumTypes[2].Descriptor()
}

func (LockStrategy) Type() protoreflect.EnumType {
	return &file_lumberjack_v1_lumberjack_proto_enumTypes[2]
}

func (x LockStrategy) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use LockStrategy.Descriptor instead.
func (LockStrategy) EnumDescriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{2}
}

// WatchResponseType identifies what a WatchResponse carries.
type WatchResponseType int32

const (
	WatchResponseType_WATCH_RESPONSE_TYPE_UNSPECIFIED WatchResponseType = 0
	// Part of the initial snapshot sent once per tracked repository right after
	// a Watch stream opens, so a client can render immediately.
	WatchResponseType_WATCH_RESPONSE_TYPE_SNAPSHOT WatchResponseType = 1
	// A worktree was created, adopted, updated, or deleted; see change.action.
	WatchResponseType_WATCH_RESPONSE_TYPE_WORKTREE_CHANGED WatchResponseType = 2
	// A repository sync started.
	WatchResponseType_WATCH_RESPONSE_TYPE_SYNC_STARTED WatchResponseType = 3
	// A repository sync finished; see summary.
	WatchResponseType_WATCH_RESPONSE_TYPE_SYNC_FINISHED WatchResponseType = 4
)

// Enum value maps for WatchResponseType.
var (
	WatchResponseType_name = map[int32]string{
		0: "WATCH_RESPONSE_TYPE_UNSPECIFIED",
		1: "WATCH_RESPONSE_TYPE_SNAPSHOT",
		2: "WATCH_RESPONSE_TYPE_WORKTREE_CHANGED",
		3: "WATCH_RESPONSE_TYPE_SYNC_STARTED",
		4: "WATCH_RESPONSE_TYPE_SYNC_FINISHED",
	}
	WatchResponseType_value = map[string]int32{
		"WATCH_RESPONSE_TYPE_UNSPECIFIED":      0,
		"WATCH_RESPONSE_TYPE_SNAPSHOT":         1,
		"WATCH_RESPONSE_TYPE_WORKTREE_CHANGED": 2,
		"WATCH_RESPONSE_TYPE_SYNC_STARTED":     3,
		"WATCH_RESPONSE_TYPE_SYNC_FINISHED":    4,
	}
)

func (x WatchResponseType) Enum() *WatchResponseType {
	p := new(WatchResponseType)
	*p = x
	return p
}

func (x WatchResponseType) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (WatchResponseType) Descriptor() protoreflect.EnumDescriptor {
	return file_lumberjack_v1_lumberjack_proto_enumTypes[3].Descriptor()
}

func (WatchResponseType) Type() protoreflect.EnumType {
	return &file_lumberjack_v1_lumberjack_proto_enumTypes[3]
}

func (x WatchResponseType) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use WatchResponseType.Descriptor instead.
func (WatchResponseType) EnumDescriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{3}
}

// WorktreeChange is one per-branch progress row: what happened to the worktree
// for a branch, the PR it relates to (absent when there is none, e.g. an
// adoption before its PR is known), and an optional human-readable detail
// (e.g. the reason a worktree was retained).
type WorktreeChange struct {
	state    protoimpl.MessageState `protogen:"open.v1"`
	Branch   string                 `protobuf:"bytes,1,opt,name=branch,proto3" json:"branch,omitempty"`
	PrNumber *int64                 `protobuf:"varint,2,opt,name=pr_number,json=prNumber,proto3,oneof" json:"pr_number,omitempty"`
	Action   WorktreeAction         `protobuf:"varint,3,opt,name=action,proto3,enum=lumberjack.v1.WorktreeAction" json:"action,omitempty"`
	Detail   string                 `protobuf:"bytes,4,opt,name=detail,proto3" json:"detail,omitempty"`
	// Absent for a DELETED change, whose directory no longer applies.
	DirectoryPath string                 `protobuf:"bytes,5,opt,name=directory_path,json=directoryPath,proto3" json:"directory_path,omitempty"`
	LastSyncedAt  *timestamppb.Timestamp `protobuf:"bytes,6,opt,name=last_synced_at,json=lastSyncedAt,proto3,oneof" json:"last_synced_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WorktreeChange) Reset() {
	*x = WorktreeChange{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WorktreeChange) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WorktreeChange) ProtoMessage() {}

func (x *WorktreeChange) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WorktreeChange.ProtoReflect.Descriptor instead.
func (*WorktreeChange) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{0}
}

func (x *WorktreeChange) GetBranch() string {
	if x != nil {
		return x.Branch
	}
	return ""
}

func (x *WorktreeChange) GetPrNumber() int64 {
	if x != nil && x.PrNumber != nil {
		return *x.PrNumber
	}
	return 0
}

func (x *WorktreeChange) GetAction() WorktreeAction {
	if x != nil {
		return x.Action
	}
	return WorktreeAction_WORKTREE_ACTION_UNSPECIFIED
}

func (x *WorktreeChange) GetDetail() string {
	if x != nil {
		return x.Detail
	}
	return ""
}

func (x *WorktreeChange) GetDirectoryPath() string {
	if x != nil {
		return x.DirectoryPath
	}
	return ""
}

func (x *WorktreeChange) GetLastSyncedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.LastSyncedAt
	}
	return nil
}

// Repository is one tracked repo. Fields mirror the `repositories` table
// (docs/schema.md); mutable PR state is never carried here.
type Repository struct {
	state             protoimpl.MessageState `protogen:"open.v1"`
	Id                int64                  `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	LocalPath         string                 `protobuf:"bytes,2,opt,name=local_path,json=localPath,proto3" json:"local_path,omitempty"`
	WorktreeParentDir string                 `protobuf:"bytes,3,opt,name=worktree_parent_dir,json=worktreeParentDir,proto3" json:"worktree_parent_dir,omitempty"`
	DirPrefix         string                 `protobuf:"bytes,4,opt,name=dir_prefix,json=dirPrefix,proto3" json:"dir_prefix,omitempty"`
	GithubOwner       string                 `protobuf:"bytes,5,opt,name=github_owner,json=githubOwner,proto3" json:"github_owner,omitempty"`
	GithubName        string                 `protobuf:"bytes,6,opt,name=github_name,json=githubName,proto3" json:"github_name,omitempty"`
	DefaultRemote     string                 `protobuf:"bytes,7,opt,name=default_remote,json=defaultRemote,proto3" json:"default_remote,omitempty"`
	Host              string                 `protobuf:"bytes,8,opt,name=host,proto3" json:"host,omitempty"`
	LastSyncedAt      *timestamppb.Timestamp `protobuf:"bytes,9,opt,name=last_synced_at,json=lastSyncedAt,proto3,oneof" json:"last_synced_at,omitempty"`
	LastSyncStatus    SyncStatus             `protobuf:"varint,10,opt,name=last_sync_status,json=lastSyncStatus,proto3,enum=lumberjack.v1.SyncStatus" json:"last_sync_status,omitempty"`
	LastSyncError     *string                `protobuf:"bytes,11,opt,name=last_sync_error,json=lastSyncError,proto3,oneof" json:"last_sync_error,omitempty"`
	CreatedAt         *timestamppb.Timestamp `protobuf:"bytes,12,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	// The gh account this repo was registered under; the daemon switches to it
	// before operating on the repo. Empty for repos tracked before login capture.
	Login string `protobuf:"bytes,13,opt,name=login,proto3" json:"login,omitempty"`
	// Deprecated: use setup_steps.is_trusted. True whenever
	// setup_steps.current_checksum is not one of setup_steps.trusted_checksums.
	//
	// Deprecated: Marked as deprecated in lumberjack/v1/lumberjack.proto.
	SetupConsentPending bool        `protobuf:"varint,14,opt,name=setup_consent_pending,json=setupConsentPending,proto3" json:"setup_consent_pending,omitempty"`
	SetupSteps          *SetupSteps `protobuf:"bytes,15,opt,name=setup_steps,json=setupSteps,proto3" json:"setup_steps,omitempty"`
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}

func (x *Repository) Reset() {
	*x = Repository{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Repository) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Repository) ProtoMessage() {}

func (x *Repository) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Repository.ProtoReflect.Descriptor instead.
func (*Repository) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{1}
}

func (x *Repository) GetId() int64 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *Repository) GetLocalPath() string {
	if x != nil {
		return x.LocalPath
	}
	return ""
}

func (x *Repository) GetWorktreeParentDir() string {
	if x != nil {
		return x.WorktreeParentDir
	}
	return ""
}

func (x *Repository) GetDirPrefix() string {
	if x != nil {
		return x.DirPrefix
	}
	return ""
}

func (x *Repository) GetGithubOwner() string {
	if x != nil {
		return x.GithubOwner
	}
	return ""
}

func (x *Repository) GetGithubName() string {
	if x != nil {
		return x.GithubName
	}
	return ""
}

func (x *Repository) GetDefaultRemote() string {
	if x != nil {
		return x.DefaultRemote
	}
	return ""
}

func (x *Repository) GetHost() string {
	if x != nil {
		return x.Host
	}
	return ""
}

func (x *Repository) GetLastSyncedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.LastSyncedAt
	}
	return nil
}

func (x *Repository) GetLastSyncStatus() SyncStatus {
	if x != nil {
		return x.LastSyncStatus
	}
	return SyncStatus_SYNC_STATUS_UNSPECIFIED
}

func (x *Repository) GetLastSyncError() string {
	if x != nil && x.LastSyncError != nil {
		return *x.LastSyncError
	}
	return ""
}

func (x *Repository) GetCreatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *Repository) GetLogin() string {
	if x != nil {
		return x.Login
	}
	return ""
}

// Deprecated: Marked as deprecated in lumberjack/v1/lumberjack.proto.
func (x *Repository) GetSetupConsentPending() bool {
	if x != nil {
		return x.SetupConsentPending
	}
	return false
}

func (x *Repository) GetSetupSteps() *SetupSteps {
	if x != nil {
		return x.SetupSteps
	}
	return nil
}

type SetupSteps struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	IsDefined        bool                   `protobuf:"varint,1,opt,name=is_defined,json=isDefined,proto3" json:"is_defined,omitempty"`
	TrustedChecksums []string               `protobuf:"bytes,2,rep,name=trusted_checksums,json=trustedChecksums,proto3" json:"trusted_checksums,omitempty"`
	CurrentChecksum  string                 `protobuf:"bytes,3,opt,name=current_checksum,json=currentChecksum,proto3" json:"current_checksum,omitempty"`
	IsTrusted        bool                   `protobuf:"varint,4,opt,name=is_trusted,json=isTrusted,proto3" json:"is_trusted,omitempty"`
	Steps            []string               `protobuf:"bytes,5,rep,name=steps,proto3" json:"steps,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *SetupSteps) Reset() {
	*x = SetupSteps{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetupSteps) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetupSteps) ProtoMessage() {}

func (x *SetupSteps) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetupSteps.ProtoReflect.Descriptor instead.
func (*SetupSteps) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{2}
}

func (x *SetupSteps) GetIsDefined() bool {
	if x != nil {
		return x.IsDefined
	}
	return false
}

func (x *SetupSteps) GetTrustedChecksums() []string {
	if x != nil {
		return x.TrustedChecksums
	}
	return nil
}

func (x *SetupSteps) GetCurrentChecksum() string {
	if x != nil {
		return x.CurrentChecksum
	}
	return ""
}

func (x *SetupSteps) GetIsTrusted() bool {
	if x != nil {
		return x.IsTrusted
	}
	return false
}

func (x *SetupSteps) GetSteps() []string {
	if x != nil {
		return x.Steps
	}
	return nil
}

// Worktree is the live view of a local worktree: the stored branch <-> directory
// mapping plus reconciliation status computed fresh from git at request time
// (never cached — see docs/schema.md "Derived live, not stored").
type Worktree struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	BranchName    string                 `protobuf:"bytes,1,opt,name=branch_name,json=branchName,proto3" json:"branch_name,omitempty"`
	DirectoryPath string                 `protobuf:"bytes,2,opt,name=directory_path,json=directoryPath,proto3" json:"directory_path,omitempty"`
	// Absent for an orphaned worktree that has outlived its PR.
	GithubPrNumber *int64                 `protobuf:"varint,3,opt,name=github_pr_number,json=githubPrNumber,proto3,oneof" json:"github_pr_number,omitempty"`
	LastSyncedAt   *timestamppb.Timestamp `protobuf:"bytes,5,opt,name=last_synced_at,json=lastSyncedAt,proto3,oneof" json:"last_synced_at,omitempty"`
	// --- derived live from git, not persisted ---
	// needs_reconciliation is true when the worktree can't be safely auto-removed
	// (dirty tree, local-only commits, or otherwise diverged).
	NeedsReconciliation bool `protobuf:"varint,6,opt,name=needs_reconciliation,json=needsReconciliation,proto3" json:"needs_reconciliation,omitempty"`
	// dirty is true when the working tree has uncommitted changes.
	Dirty bool `protobuf:"varint,7,opt,name=dirty,proto3" json:"dirty,omitempty"`
	// orphaned is true when the PR has been merged/closed but the worktree was
	// retained because it still holds unmerged local work.
	Orphaned bool `protobuf:"varint,8,opt,name=orphaned,proto3" json:"orphaned,omitempty"`
	// local_only_commits counts commits present locally but not in the merged
	// remote branch — the count surfaced when confirming a delete.
	LocalOnlyCommits int64 `protobuf:"varint,9,opt,name=local_only_commits,json=localOnlyCommits,proto3" json:"local_only_commits,omitempty"`
	// reconciliation_note is a human-readable summary; empty when in sync.
	ReconciliationNote string `protobuf:"bytes,10,opt,name=reconciliation_note,json=reconciliationNote,proto3" json:"reconciliation_note,omitempty"`
	// branch_disparity is true when the branch checked out in the directory is
	// not the branch of its pull request. Such a worktree is never cleaned up
	// automatically, and no worktree is created for a PR whose branch is checked
	// out somewhere else.
	BranchDisparity bool `protobuf:"varint,11,opt,name=branch_disparity,json=branchDisparity,proto3" json:"branch_disparity,omitempty"`
	// checked_out_branch is the branch git reports in the directory, empty for a
	// detached HEAD. It differs from branch_name exactly when branch_disparity.
	CheckedOutBranch string `protobuf:"bytes,12,opt,name=checked_out_branch,json=checkedOutBranch,proto3" json:"checked_out_branch,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *Worktree) Reset() {
	*x = Worktree{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Worktree) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Worktree) ProtoMessage() {}

func (x *Worktree) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Worktree.ProtoReflect.Descriptor instead.
func (*Worktree) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{3}
}

func (x *Worktree) GetBranchName() string {
	if x != nil {
		return x.BranchName
	}
	return ""
}

func (x *Worktree) GetDirectoryPath() string {
	if x != nil {
		return x.DirectoryPath
	}
	return ""
}

func (x *Worktree) GetGithubPrNumber() int64 {
	if x != nil && x.GithubPrNumber != nil {
		return *x.GithubPrNumber
	}
	return 0
}

func (x *Worktree) GetLastSyncedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.LastSyncedAt
	}
	return nil
}

func (x *Worktree) GetNeedsReconciliation() bool {
	if x != nil {
		return x.NeedsReconciliation
	}
	return false
}

func (x *Worktree) GetDirty() bool {
	if x != nil {
		return x.Dirty
	}
	return false
}

func (x *Worktree) GetOrphaned() bool {
	if x != nil {
		return x.Orphaned
	}
	return false
}

func (x *Worktree) GetLocalOnlyCommits() int64 {
	if x != nil {
		return x.LocalOnlyCommits
	}
	return 0
}

func (x *Worktree) GetReconciliationNote() string {
	if x != nil {
		return x.ReconciliationNote
	}
	return ""
}

func (x *Worktree) GetBranchDisparity() bool {
	if x != nil {
		return x.BranchDisparity
	}
	return false
}

func (x *Worktree) GetCheckedOutBranch() string {
	if x != nil {
		return x.CheckedOutBranch
	}
	return ""
}

type HealthRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *HealthRequest) Reset() {
	*x = HealthRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HealthRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HealthRequest) ProtoMessage() {}

func (x *HealthRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HealthRequest.ProtoReflect.Descriptor instead.
func (*HealthRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{4}
}

type HealthResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Version       string                 `protobuf:"bytes,1,opt,name=version,proto3" json:"version,omitempty"`
	StartedAt     *timestamppb.Timestamp `protobuf:"bytes,2,opt,name=started_at,json=startedAt,proto3" json:"started_at,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *HealthResponse) Reset() {
	*x = HealthResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *HealthResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*HealthResponse) ProtoMessage() {}

func (x *HealthResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use HealthResponse.ProtoReflect.Descriptor instead.
func (*HealthResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{5}
}

func (x *HealthResponse) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *HealthResponse) GetStartedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.StartedAt
	}
	return nil
}

type InitRepositoryRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Absolute path to the repo being registered (`lumberjack init .` resolves
	// "." to the current directory before calling).
	LocalPath     string `protobuf:"bytes,1,opt,name=local_path,json=localPath,proto3" json:"local_path,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *InitRepositoryRequest) Reset() {
	*x = InitRepositoryRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *InitRepositoryRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*InitRepositoryRequest) ProtoMessage() {}

func (x *InitRepositoryRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use InitRepositoryRequest.ProtoReflect.Descriptor instead.
func (*InitRepositoryRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{6}
}

func (x *InitRepositoryRequest) GetLocalPath() string {
	if x != nil {
		return x.LocalPath
	}
	return ""
}

type InitRepositoryResponse struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	Repository *Repository            `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	// Deprecated: superseded by `adopted` (field 3), which carries the full
	// per-branch change so init can render the same branch/PR/action table as
	// sync. No longer populated by the daemon.
	//
	// Deprecated: Marked as deprecated in lumberjack/v1/lumberjack.proto.
	AdoptedBranches []string `protobuf:"bytes,2,rep,name=adopted_branches,json=adoptedBranches,proto3" json:"adopted_branches,omitempty"`
	// The worktrees adopted into tracking during init — directories git had
	// already checked out for this repo but Lumberjack did not yet track (each
	// with action ADOPTED and no PR number yet). Empty for a fresh checkout with
	// no sibling worktrees.
	Adopted       []*WorktreeChange `protobuf:"bytes,3,rep,name=adopted,proto3" json:"adopted,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *InitRepositoryResponse) Reset() {
	*x = InitRepositoryResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *InitRepositoryResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*InitRepositoryResponse) ProtoMessage() {}

func (x *InitRepositoryResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use InitRepositoryResponse.ProtoReflect.Descriptor instead.
func (*InitRepositoryResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{7}
}

func (x *InitRepositoryResponse) GetRepository() *Repository {
	if x != nil {
		return x.Repository
	}
	return nil
}

// Deprecated: Marked as deprecated in lumberjack/v1/lumberjack.proto.
func (x *InitRepositoryResponse) GetAdoptedBranches() []string {
	if x != nil {
		return x.AdoptedBranches
	}
	return nil
}

func (x *InitRepositoryResponse) GetAdopted() []*WorktreeChange {
	if x != nil {
		return x.Adopted
	}
	return nil
}

type ListRepositoriesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRepositoriesRequest) Reset() {
	*x = ListRepositoriesRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRepositoriesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRepositoriesRequest) ProtoMessage() {}

func (x *ListRepositoriesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRepositoriesRequest.ProtoReflect.Descriptor instead.
func (*ListRepositoriesRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{8}
}

type ListRepositoriesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Repositories  []*Repository          `protobuf:"bytes,1,rep,name=repositories,proto3" json:"repositories,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListRepositoriesResponse) Reset() {
	*x = ListRepositoriesResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListRepositoriesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRepositoriesResponse) ProtoMessage() {}

func (x *ListRepositoriesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRepositoriesResponse.ProtoReflect.Descriptor instead.
func (*ListRepositoriesResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{9}
}

func (x *ListRepositoriesResponse) GetRepositories() []*Repository {
	if x != nil {
		return x.Repositories
	}
	return nil
}

type GetRepositoryRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// A repository name (dir prefix / GitHub name) or its local path; the daemon
	// resolves either form.
	Repository    string `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRepositoryRequest) Reset() {
	*x = GetRepositoryRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRepositoryRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRepositoryRequest) ProtoMessage() {}

func (x *GetRepositoryRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRepositoryRequest.ProtoReflect.Descriptor instead.
func (*GetRepositoryRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{10}
}

func (x *GetRepositoryRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

type GetRepositoryResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Repository    *Repository            `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetRepositoryResponse) Reset() {
	*x = GetRepositoryResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetRepositoryResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetRepositoryResponse) ProtoMessage() {}

func (x *GetRepositoryResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetRepositoryResponse.ProtoReflect.Descriptor instead.
func (*GetRepositoryResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{11}
}

func (x *GetRepositoryResponse) GetRepository() *Repository {
	if x != nil {
		return x.Repository
	}
	return nil
}

type SetLoginRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// A repository name (dir prefix / GitHub name) or its local path; the daemon
	// resolves either form.
	Repository string `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	// The gh account login the repository should operate under henceforth.
	Login         string `protobuf:"bytes,2,opt,name=login,proto3" json:"login,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetLoginRequest) Reset() {
	*x = SetLoginRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetLoginRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetLoginRequest) ProtoMessage() {}

func (x *SetLoginRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetLoginRequest.ProtoReflect.Descriptor instead.
func (*SetLoginRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{12}
}

func (x *SetLoginRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

func (x *SetLoginRequest) GetLogin() string {
	if x != nil {
		return x.Login
	}
	return ""
}

type SetLoginResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Repository    *Repository            `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetLoginResponse) Reset() {
	*x = SetLoginResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetLoginResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetLoginResponse) ProtoMessage() {}

func (x *SetLoginResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetLoginResponse.ProtoReflect.Descriptor instead.
func (*SetLoginResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{13}
}

func (x *SetLoginResponse) GetRepository() *Repository {
	if x != nil {
		return x.Repository
	}
	return nil
}

type ListLoginsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// A repository name (dir prefix / GitHub name) or its local path; the daemon
	// resolves either form and lists the gh accounts for its host.
	Repository    string `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListLoginsRequest) Reset() {
	*x = ListLoginsRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListLoginsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListLoginsRequest) ProtoMessage() {}

func (x *ListLoginsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListLoginsRequest.ProtoReflect.Descriptor instead.
func (*ListLoginsRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{14}
}

func (x *ListLoginsRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

type ListLoginsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The gh account logins authenticated for the repository's host.
	Logins []string `protobuf:"bytes,1,rep,name=logins,proto3" json:"logins,omitempty"`
	// The login the repository currently operates under, if any — the CLI
	// pre-selects it in the picker. Empty for repos tracked before login capture.
	Current       string `protobuf:"bytes,2,opt,name=current,proto3" json:"current,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListLoginsResponse) Reset() {
	*x = ListLoginsResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListLoginsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListLoginsResponse) ProtoMessage() {}

func (x *ListLoginsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListLoginsResponse.ProtoReflect.Descriptor instead.
func (*ListLoginsResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{15}
}

func (x *ListLoginsResponse) GetLogins() []string {
	if x != nil {
		return x.Logins
	}
	return nil
}

func (x *ListLoginsResponse) GetCurrent() string {
	if x != nil {
		return x.Current
	}
	return ""
}

// Deprecated: read Repository.setup_steps instead.
//
// Deprecated: Marked as deprecated in lumberjack/v1/lumberjack.proto.
type GetSetupConsentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// A repository name (dir prefix / GitHub name) or its local path; the daemon
	// resolves either form.
	Repository    string `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSetupConsentRequest) Reset() {
	*x = GetSetupConsentRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSetupConsentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSetupConsentRequest) ProtoMessage() {}

func (x *GetSetupConsentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSetupConsentRequest.ProtoReflect.Descriptor instead.
func (*GetSetupConsentRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{16}
}

func (x *GetSetupConsentRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

// Deprecated: read Repository.setup_steps instead.
//
// Deprecated: Marked as deprecated in lumberjack/v1/lumberjack.proto.
type GetSetupConsentResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// True when the repository's trusted `.lumberjack.yml` declares run-command
	// steps the local user has not (yet, or still) consented to.
	Pending bool `protobuf:"varint,1,opt,name=pending,proto3" json:"pending,omitempty"`
	// The run-command strings from the trusted config, for the consent prompt.
	// Empty when pending is false.
	RunCommands   []string `protobuf:"bytes,2,rep,name=run_commands,json=runCommands,proto3" json:"run_commands,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetSetupConsentResponse) Reset() {
	*x = GetSetupConsentResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetSetupConsentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetSetupConsentResponse) ProtoMessage() {}

func (x *GetSetupConsentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetSetupConsentResponse.ProtoReflect.Descriptor instead.
func (*GetSetupConsentResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{17}
}

func (x *GetSetupConsentResponse) GetPending() bool {
	if x != nil {
		return x.Pending
	}
	return false
}

func (x *GetSetupConsentResponse) GetRunCommands() []string {
	if x != nil {
		return x.RunCommands
	}
	return nil
}

type SetSetupConsentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// A repository name (dir prefix / GitHub name) or its local path; the daemon
	// resolves either form.
	Repository    string `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	Checksum      string `protobuf:"bytes,2,opt,name=checksum,proto3" json:"checksum,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetSetupConsentRequest) Reset() {
	*x = SetSetupConsentRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetSetupConsentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetSetupConsentRequest) ProtoMessage() {}

func (x *SetSetupConsentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetSetupConsentRequest.ProtoReflect.Descriptor instead.
func (*SetSetupConsentRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{18}
}

func (x *SetSetupConsentRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

func (x *SetSetupConsentRequest) GetChecksum() string {
	if x != nil {
		return x.Checksum
	}
	return ""
}

type SetSetupConsentResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Repository    *Repository            `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	Accepted      bool                   `protobuf:"varint,2,opt,name=accepted,proto3" json:"accepted,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SetSetupConsentResponse) Reset() {
	*x = SetSetupConsentResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SetSetupConsentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SetSetupConsentResponse) ProtoMessage() {}

func (x *SetSetupConsentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SetSetupConsentResponse.ProtoReflect.Descriptor instead.
func (*SetSetupConsentResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{19}
}

func (x *SetSetupConsentResponse) GetRepository() *Repository {
	if x != nil {
		return x.Repository
	}
	return nil
}

func (x *SetSetupConsentResponse) GetAccepted() bool {
	if x != nil {
		return x.Accepted
	}
	return false
}

type TrustSetupStepsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// A repository name (dir prefix / GitHub name) or its local path; the daemon
	// resolves either form.
	Repository    string `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	Checksum      string `protobuf:"bytes,2,opt,name=checksum,proto3" json:"checksum,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TrustSetupStepsRequest) Reset() {
	*x = TrustSetupStepsRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TrustSetupStepsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TrustSetupStepsRequest) ProtoMessage() {}

func (x *TrustSetupStepsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TrustSetupStepsRequest.ProtoReflect.Descriptor instead.
func (*TrustSetupStepsRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{20}
}

func (x *TrustSetupStepsRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

func (x *TrustSetupStepsRequest) GetChecksum() string {
	if x != nil {
		return x.Checksum
	}
	return ""
}

type TrustSetupStepsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Repository    *Repository            `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TrustSetupStepsResponse) Reset() {
	*x = TrustSetupStepsResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TrustSetupStepsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TrustSetupStepsResponse) ProtoMessage() {}

func (x *TrustSetupStepsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TrustSetupStepsResponse.ProtoReflect.Descriptor instead.
func (*TrustSetupStepsResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{21}
}

func (x *TrustSetupStepsResponse) GetRepository() *Repository {
	if x != nil {
		return x.Repository
	}
	return nil
}

type ListWorktreesRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Repository    string                 `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListWorktreesRequest) Reset() {
	*x = ListWorktreesRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListWorktreesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListWorktreesRequest) ProtoMessage() {}

func (x *ListWorktreesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListWorktreesRequest.ProtoReflect.Descriptor instead.
func (*ListWorktreesRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{22}
}

func (x *ListWorktreesRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

type ListWorktreesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Worktrees     []*Worktree            `protobuf:"bytes,1,rep,name=worktrees,proto3" json:"worktrees,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListWorktreesResponse) Reset() {
	*x = ListWorktreesResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListWorktreesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListWorktreesResponse) ProtoMessage() {}

func (x *ListWorktreesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListWorktreesResponse.ProtoReflect.Descriptor instead.
func (*ListWorktreesResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{23}
}

func (x *ListWorktreesResponse) GetWorktrees() []*Worktree {
	if x != nil {
		return x.Worktrees
	}
	return nil
}

type AddWorktreeRequest struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	Repository string                 `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	// The branch to check out. Created off the repository's default branch when
	// it exists neither on the remote nor locally.
	Branch        string `protobuf:"bytes,2,opt,name=branch,proto3" json:"branch,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AddWorktreeRequest) Reset() {
	*x = AddWorktreeRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AddWorktreeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddWorktreeRequest) ProtoMessage() {}

func (x *AddWorktreeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddWorktreeRequest.ProtoReflect.Descriptor instead.
func (*AddWorktreeRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{24}
}

func (x *AddWorktreeRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

func (x *AddWorktreeRequest) GetBranch() string {
	if x != nil {
		return x.Branch
	}
	return ""
}

type AddWorktreeResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Absolute path of the created worktree.
	DirectoryPath string `protobuf:"bytes,1,opt,name=directory_path,json=directoryPath,proto3" json:"directory_path,omitempty"`
	// The branch checked out there.
	Branch string `protobuf:"bytes,2,opt,name=branch,proto3" json:"branch,omitempty"`
	// True when the branch was created for this worktree rather than checked out
	// from an existing remote or local branch.
	BranchCreated bool `protobuf:"varint,3,opt,name=branch_created,json=branchCreated,proto3" json:"branch_created,omitempty"`
	// The setup-step failure, if any. The worktree is created and tracked
	// regardless (setup failures are surfaced, not fatal); empty on success or
	// when the repository declares no setup steps.
	SetupError    string `protobuf:"bytes,4,opt,name=setup_error,json=setupError,proto3" json:"setup_error,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AddWorktreeResponse) Reset() {
	*x = AddWorktreeResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AddWorktreeResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AddWorktreeResponse) ProtoMessage() {}

func (x *AddWorktreeResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AddWorktreeResponse.ProtoReflect.Descriptor instead.
func (*AddWorktreeResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{25}
}

func (x *AddWorktreeResponse) GetDirectoryPath() string {
	if x != nil {
		return x.DirectoryPath
	}
	return ""
}

func (x *AddWorktreeResponse) GetBranch() string {
	if x != nil {
		return x.Branch
	}
	return ""
}

func (x *AddWorktreeResponse) GetBranchCreated() bool {
	if x != nil {
		return x.BranchCreated
	}
	return false
}

func (x *AddWorktreeResponse) GetSetupError() string {
	if x != nil {
		return x.SetupError
	}
	return ""
}

type DeleteWorktreeRequest struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	Repository string                 `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	// Branch name or directory name identifying the worktree.
	Worktree string `protobuf:"bytes,2,opt,name=worktree,proto3" json:"worktree,omitempty"`
	// Proceed even when local commits would be lost. The CLI sets this only after
	// the user confirms the warning returned by a first, unforced attempt.
	Force         bool `protobuf:"varint,3,opt,name=force,proto3" json:"force,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteWorktreeRequest) Reset() {
	*x = DeleteWorktreeRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteWorktreeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteWorktreeRequest) ProtoMessage() {}

func (x *DeleteWorktreeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteWorktreeRequest.ProtoReflect.Descriptor instead.
func (*DeleteWorktreeRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{26}
}

func (x *DeleteWorktreeRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

func (x *DeleteWorktreeRequest) GetWorktree() string {
	if x != nil {
		return x.Worktree
	}
	return ""
}

func (x *DeleteWorktreeRequest) GetForce() bool {
	if x != nil {
		return x.Force
	}
	return false
}

type DeleteWorktreeResponse struct {
	state   protoimpl.MessageState `protogen:"open.v1"`
	Deleted bool                   `protobuf:"varint,1,opt,name=deleted,proto3" json:"deleted,omitempty"`
	// True when the worktree's local tip is ahead of the merged remote: the CLI
	// must warn the user and retry with force=true to actually delete.
	RequiresConfirmation bool `protobuf:"varint,2,opt,name=requires_confirmation,json=requiresConfirmation,proto3" json:"requires_confirmation,omitempty"`
	// Number of commits that would be lost, for the confirmation warning.
	CommitsAtRisk int64  `protobuf:"varint,3,opt,name=commits_at_risk,json=commitsAtRisk,proto3" json:"commits_at_risk,omitempty"`
	Message       string `protobuf:"bytes,4,opt,name=message,proto3" json:"message,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteWorktreeResponse) Reset() {
	*x = DeleteWorktreeResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteWorktreeResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteWorktreeResponse) ProtoMessage() {}

func (x *DeleteWorktreeResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteWorktreeResponse.ProtoReflect.Descriptor instead.
func (*DeleteWorktreeResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{27}
}

func (x *DeleteWorktreeResponse) GetDeleted() bool {
	if x != nil {
		return x.Deleted
	}
	return false
}

func (x *DeleteWorktreeResponse) GetRequiresConfirmation() bool {
	if x != nil {
		return x.RequiresConfirmation
	}
	return false
}

func (x *DeleteWorktreeResponse) GetCommitsAtRisk() int64 {
	if x != nil {
		return x.CommitsAtRisk
	}
	return 0
}

func (x *DeleteWorktreeResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

type DeleteRepositoryRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// A repository name (dir prefix / GitHub name) or its local path; the daemon
	// resolves either form.
	Repository    string `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteRepositoryRequest) Reset() {
	*x = DeleteRepositoryRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteRepositoryRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteRepositoryRequest) ProtoMessage() {}

func (x *DeleteRepositoryRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteRepositoryRequest.ProtoReflect.Descriptor instead.
func (*DeleteRepositoryRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{28}
}

func (x *DeleteRepositoryRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

type DeleteRepositoryResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Number of worktree rows removed alongside the repository.
	WorktreesRemoved int64  `protobuf:"varint,1,opt,name=worktrees_removed,json=worktreesRemoved,proto3" json:"worktrees_removed,omitempty"`
	Message          string `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *DeleteRepositoryResponse) Reset() {
	*x = DeleteRepositoryResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteRepositoryResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteRepositoryResponse) ProtoMessage() {}

func (x *DeleteRepositoryResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteRepositoryResponse.ProtoReflect.Descriptor instead.
func (*DeleteRepositoryResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{29}
}

func (x *DeleteRepositoryResponse) GetWorktreesRemoved() int64 {
	if x != nil {
		return x.WorktreesRemoved
	}
	return 0
}

func (x *DeleteRepositoryResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

type SyncRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Empty syncs every tracked repository; set syncs just that one.
	Repository    string `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SyncRequest) Reset() {
	*x = SyncRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SyncRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SyncRequest) ProtoMessage() {}

func (x *SyncRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SyncRequest.ProtoReflect.Descriptor instead.
func (*SyncRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{30}
}

func (x *SyncRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

// SyncResponse is one progress update in a Sync stream. Multiple events may be
// emitted per repository; the last one for a repo has completed = true and
// carries its summary.
type SyncResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Which repository this event concerns.
	Repository string `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	// Human-readable progress line, for events that are not a per-branch worktree
	// change (worktree changes use `change` instead).
	Message string `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
	// True on the final event for this repository.
	Completed bool `protobuf:"varint,3,opt,name=completed,proto3" json:"completed,omitempty"`
	// Populated when completed = true.
	Summary *SyncSummary `protobuf:"bytes,4,opt,name=summary,proto3" json:"summary,omitempty"`
	// A per-branch worktree change (checked out / adopted / updated / deleted /
	// retained), for progress events that concern one branch. Absent on the
	// completion event and on plain `message` events.
	Change        *WorktreeChange `protobuf:"bytes,5,opt,name=change,proto3" json:"change,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SyncResponse) Reset() {
	*x = SyncResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SyncResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SyncResponse) ProtoMessage() {}

func (x *SyncResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SyncResponse.ProtoReflect.Descriptor instead.
func (*SyncResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{31}
}

func (x *SyncResponse) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

func (x *SyncResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *SyncResponse) GetCompleted() bool {
	if x != nil {
		return x.Completed
	}
	return false
}

func (x *SyncResponse) GetSummary() *SyncSummary {
	if x != nil {
		return x.Summary
	}
	return nil
}

func (x *SyncResponse) GetChange() *WorktreeChange {
	if x != nil {
		return x.Change
	}
	return nil
}

type SyncSummary struct {
	state            protoimpl.MessageState `protogen:"open.v1"`
	Status           SyncStatus             `protobuf:"varint,1,opt,name=status,proto3,enum=lumberjack.v1.SyncStatus" json:"status,omitempty"`
	WorktreesCreated int64                  `protobuf:"varint,2,opt,name=worktrees_created,json=worktreesCreated,proto3" json:"worktrees_created,omitempty"`
	WorktreesRemoved int64                  `protobuf:"varint,3,opt,name=worktrees_removed,json=worktreesRemoved,proto3" json:"worktrees_removed,omitempty"`
	Error            *string                `protobuf:"bytes,4,opt,name=error,proto3,oneof" json:"error,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}

func (x *SyncSummary) Reset() {
	*x = SyncSummary{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SyncSummary) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SyncSummary) ProtoMessage() {}

func (x *SyncSummary) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SyncSummary.ProtoReflect.Descriptor instead.
func (*SyncSummary) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{32}
}

func (x *SyncSummary) GetStatus() SyncStatus {
	if x != nil {
		return x.Status
	}
	return SyncStatus_SYNC_STATUS_UNSPECIFIED
}

func (x *SyncSummary) GetWorktreesCreated() int64 {
	if x != nil {
		return x.WorktreesCreated
	}
	return 0
}

func (x *SyncSummary) GetWorktreesRemoved() int64 {
	if x != nil {
		return x.WorktreesRemoved
	}
	return 0
}

func (x *SyncSummary) GetError() string {
	if x != nil && x.Error != nil {
		return *x.Error
	}
	return ""
}

// LockDecision is a strategy chosen for one specific locked worktree, as the
// CLI's interactive prompt produces (one question per locked worktree). It
// overrides TidyRequest.lock_strategy for that worktree only.
type LockDecision struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The worktree's current directory — TidyMove.from, which is unique across
	// repositories and is what the prompt named.
	WorktreePath string `protobuf:"bytes,1,opt,name=worktree_path,json=worktreePath,proto3" json:"worktree_path,omitempty"`
	// What to do with that worktree's lock. UNSPECIFIED is treated as SKIP.
	Strategy      LockStrategy `protobuf:"varint,2,opt,name=strategy,proto3,enum=lumberjack.v1.LockStrategy" json:"strategy,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *LockDecision) Reset() {
	*x = LockDecision{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LockDecision) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LockDecision) ProtoMessage() {}

func (x *LockDecision) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use LockDecision.ProtoReflect.Descriptor instead.
func (*LockDecision) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{33}
}

func (x *LockDecision) GetWorktreePath() string {
	if x != nil {
		return x.WorktreePath
	}
	return ""
}

func (x *LockDecision) GetStrategy() LockStrategy {
	if x != nil {
		return x.Strategy
	}
	return LockStrategy_LOCK_STRATEGY_UNSPECIFIED
}

type TidyRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Empty tidies every tracked repository; set tidies just that one.
	Repository string `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	// Report what would move without touching anything on disk or in the
	// database.
	DryRun bool `protobuf:"varint,2,opt,name=dry_run,json=dryRun,proto3" json:"dry_run,omitempty"`
	// Restrict the tidy to one worktree, named by its branch or its directory
	// (full path or base name) — the same reference DeleteWorktree accepts.
	// Empty considers every worktree in scope. The daemon reports NOT_FOUND
	// when nothing matches. Whether the named worktree can move still depends
	// on every other worktree in its repository, since none may be moved onto a
	// path another already holds.
	Worktree string `protobuf:"bytes,3,opt,name=worktree,proto3" json:"worktree,omitempty"`
	// What to do with a locked worktree that has no lock_decision of its own.
	LockStrategy LockStrategy `protobuf:"varint,4,opt,name=lock_strategy,json=lockStrategy,proto3,enum=lumberjack.v1.LockStrategy" json:"lock_strategy,omitempty"`
	// Per-worktree overrides of lock_strategy, at most one per worktree path. A
	// path that is not (or is no longer) a locked misplaced worktree is ignored.
	LockDecisions []*LockDecision `protobuf:"bytes,5,rep,name=lock_decisions,json=lockDecisions,proto3" json:"lock_decisions,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TidyRequest) Reset() {
	*x = TidyRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TidyRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TidyRequest) ProtoMessage() {}

func (x *TidyRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TidyRequest.ProtoReflect.Descriptor instead.
func (*TidyRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{34}
}

func (x *TidyRequest) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

func (x *TidyRequest) GetDryRun() bool {
	if x != nil {
		return x.DryRun
	}
	return false
}

func (x *TidyRequest) GetWorktree() string {
	if x != nil {
		return x.Worktree
	}
	return ""
}

func (x *TidyRequest) GetLockStrategy() LockStrategy {
	if x != nil {
		return x.LockStrategy
	}
	return LockStrategy_LOCK_STRATEGY_UNSPECIFIED
}

func (x *TidyRequest) GetLockDecisions() []*LockDecision {
	if x != nil {
		return x.LockDecisions
	}
	return nil
}

// TidyMove is one tracked worktree found outside its idiomatic location. Every
// misplaced worktree yields exactly one of these, whether it was moved, only
// reported (dry run), or could not be moved (error set).
type TidyMove struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The repository the worktree belongs to, so `tidy-all` output can be
	// attributed.
	Repository string `protobuf:"bytes,1,opt,name=repository,proto3" json:"repository,omitempty"`
	Branch     string `protobuf:"bytes,2,opt,name=branch,proto3" json:"branch,omitempty"`
	// Where the worktree is now.
	From string `protobuf:"bytes,3,opt,name=from,proto3" json:"from,omitempty"`
	// Where the naming convention says it belongs.
	To string `protobuf:"bytes,4,opt,name=to,proto3" json:"to,omitempty"`
	// True when the worktree was actually moved; false for a dry run and for a
	// move that failed.
	Moved bool `protobuf:"varint,5,opt,name=moved,proto3" json:"moved,omitempty"`
	// Why the move was not made (e.g. the destination is occupied, or the
	// worktree is locked and the strategy was SKIP). Empty when moved, or on a
	// dry run with nothing standing in the way.
	Error string `protobuf:"bytes,6,opt,name=error,proto3" json:"error,omitempty"`
	// True when git had the worktree locked when tidy looked. It stays true for
	// a worktree that was unlocked, moved and re-locked — it describes what tidy
	// found, which is what a dry run needs to report so the CLI knows which
	// worktrees to prompt about.
	Locked bool `protobuf:"varint,7,opt,name=locked,proto3" json:"locked,omitempty"`
	// The reason git recorded with the lock, empty when the lock has none (or
	// the worktree is not locked).
	LockReason    string `protobuf:"bytes,8,opt,name=lock_reason,json=lockReason,proto3" json:"lock_reason,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TidyMove) Reset() {
	*x = TidyMove{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[35]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TidyMove) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TidyMove) ProtoMessage() {}

func (x *TidyMove) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[35]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TidyMove.ProtoReflect.Descriptor instead.
func (*TidyMove) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{35}
}

func (x *TidyMove) GetRepository() string {
	if x != nil {
		return x.Repository
	}
	return ""
}

func (x *TidyMove) GetBranch() string {
	if x != nil {
		return x.Branch
	}
	return ""
}

func (x *TidyMove) GetFrom() string {
	if x != nil {
		return x.From
	}
	return ""
}

func (x *TidyMove) GetTo() string {
	if x != nil {
		return x.To
	}
	return ""
}

func (x *TidyMove) GetMoved() bool {
	if x != nil {
		return x.Moved
	}
	return false
}

func (x *TidyMove) GetError() string {
	if x != nil {
		return x.Error
	}
	return ""
}

func (x *TidyMove) GetLocked() bool {
	if x != nil {
		return x.Locked
	}
	return false
}

func (x *TidyMove) GetLockReason() string {
	if x != nil {
		return x.LockReason
	}
	return ""
}

type TidyResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// One entry per misplaced worktree; empty when everything is already in
	// its idiomatic location.
	Moves         []*TidyMove `protobuf:"bytes,1,rep,name=moves,proto3" json:"moves,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TidyResponse) Reset() {
	*x = TidyResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[36]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TidyResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TidyResponse) ProtoMessage() {}

func (x *TidyResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[36]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TidyResponse.ProtoReflect.Descriptor instead.
func (*TidyResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{36}
}

func (x *TidyResponse) GetMoves() []*TidyMove {
	if x != nil {
		return x.Moves
	}
	return nil
}

type WatchRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WatchRequest) Reset() {
	*x = WatchRequest{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[37]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WatchRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WatchRequest) ProtoMessage() {}

func (x *WatchRequest) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[37]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WatchRequest.ProtoReflect.Descriptor instead.
func (*WatchRequest) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{37}
}

// WatchResponse is one item in a Watch stream. Every event carries the affected
// repository; which other field is populated depends on type.
type WatchResponse struct {
	state      protoimpl.MessageState `protogen:"open.v1"`
	Type       WatchResponseType      `protobuf:"varint,1,opt,name=type,proto3,enum=lumberjack.v1.WatchResponseType" json:"type,omitempty"`
	Repository *Repository            `protobuf:"bytes,2,opt,name=repository,proto3" json:"repository,omitempty"`
	// Populated for SNAPSHOT: the repository's current worktrees.
	Worktrees []*Worktree `protobuf:"bytes,3,rep,name=worktrees,proto3" json:"worktrees,omitempty"`
	// Populated for WORKTREE_CHANGED.
	Change *WorktreeChange `protobuf:"bytes,4,opt,name=change,proto3" json:"change,omitempty"`
	// Populated for SYNC_FINISHED.
	Summary       *SyncSummary `protobuf:"bytes,5,opt,name=summary,proto3" json:"summary,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *WatchResponse) Reset() {
	*x = WatchResponse{}
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[38]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *WatchResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*WatchResponse) ProtoMessage() {}

func (x *WatchResponse) ProtoReflect() protoreflect.Message {
	mi := &file_lumberjack_v1_lumberjack_proto_msgTypes[38]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use WatchResponse.ProtoReflect.Descriptor instead.
func (*WatchResponse) Descriptor() ([]byte, []int) {
	return file_lumberjack_v1_lumberjack_proto_rawDescGZIP(), []int{38}
}

func (x *WatchResponse) GetType() WatchResponseType {
	if x != nil {
		return x.Type
	}
	return WatchResponseType_WATCH_RESPONSE_TYPE_UNSPECIFIED
}

func (x *WatchResponse) GetRepository() *Repository {
	if x != nil {
		return x.Repository
	}
	return nil
}

func (x *WatchResponse) GetWorktrees() []*Worktree {
	if x != nil {
		return x.Worktrees
	}
	return nil
}

func (x *WatchResponse) GetChange() *WorktreeChange {
	if x != nil {
		return x.Change
	}
	return nil
}

func (x *WatchResponse) GetSummary() *SyncSummary {
	if x != nil {
		return x.Summary
	}
	return nil
}

var File_lumberjack_v1_lumberjack_proto protoreflect.FileDescriptor

const file_lumberjack_v1_lumberjack_proto_rawDesc = "" +
	"\n" +
	"\x1elumberjack/v1/lumberjack.proto\x12\rlumberjack.v1\x1a\x1fgoogle/protobuf/timestamp.proto\"\xa8\x02\n" +
	"\x0eWorktreeChange\x12\x16\n" +
	"\x06branch\x18\x01 \x01(\tR\x06branch\x12 \n" +
	"\tpr_number\x18\x02 \x01(\x03H\x00R\bprNumber\x88\x01\x01\x125\n" +
	"\x06action\x18\x03 \x01(\x0e2\x1d.lumberjack.v1.WorktreeActionR\x06action\x12\x16\n" +
	"\x06detail\x18\x04 \x01(\tR\x06detail\x12%\n" +
	"\x0edirectory_path\x18\x05 \x01(\tR\rdirectoryPath\x12E\n" +
	"\x0elast_synced_at\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampH\x01R\flastSyncedAt\x88\x01\x01B\f\n" +
	"\n" +
	"_pr_numberB\x11\n" +
	"\x0f_last_synced_at\"\xae\x05\n" +
	"\n" +
	"Repository\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\x03R\x02id\x12\x1d\n" +
	"\n" +
	"local_path\x18\x02 \x01(\tR\tlocalPath\x12.\n" +
	"\x13worktree_parent_dir\x18\x03 \x01(\tR\x11worktreeParentDir\x12\x1d\n" +
	"\n" +
	"dir_prefix\x18\x04 \x01(\tR\tdirPrefix\x12!\n" +
	"\fgithub_owner\x18\x05 \x01(\tR\vgithubOwner\x12\x1f\n" +
	"\vgithub_name\x18\x06 \x01(\tR\n" +
	"githubName\x12%\n" +
	"\x0edefault_remote\x18\a \x01(\tR\rdefaultRemote\x12\x12\n" +
	"\x04host\x18\b \x01(\tR\x04host\x12E\n" +
	"\x0elast_synced_at\x18\t \x01(\v2\x1a.google.protobuf.TimestampH\x00R\flastSyncedAt\x88\x01\x01\x12C\n" +
	"\x10last_sync_status\x18\n" +
	" \x01(\x0e2\x19.lumberjack.v1.SyncStatusR\x0elastSyncStatus\x12+\n" +
	"\x0flast_sync_error\x18\v \x01(\tH\x01R\rlastSyncError\x88\x01\x01\x129\n" +
	"\n" +
	"created_at\x18\f \x01(\v2\x1a.google.protobuf.TimestampR\tcreatedAt\x12\x14\n" +
	"\x05login\x18\r \x01(\tR\x05login\x126\n" +
	"\x15setup_consent_pending\x18\x0e \x01(\bB\x02\x18\x01R\x13setupConsentPending\x12:\n" +
	"\vsetup_steps\x18\x0f \x01(\v2\x19.lumberjack.v1.SetupStepsR\n" +
	"setupStepsB\x11\n" +
	"\x0f_last_synced_atB\x12\n" +
	"\x10_last_sync_error\"\xb8\x01\n" +
	"\n" +
	"SetupSteps\x12\x1d\n" +
	"\n" +
	"is_defined\x18\x01 \x01(\bR\tisDefined\x12+\n" +
	"\x11trusted_checksums\x18\x02 \x03(\tR\x10trustedChecksums\x12)\n" +
	"\x10current_checksum\x18\x03 \x01(\tR\x0fcurrentChecksum\x12\x1d\n" +
	"\n" +
	"is_trusted\x18\x04 \x01(\bR\tisTrusted\x12\x14\n" +
	"\x05steps\x18\x05 \x03(\tR\x05steps\"\x9f\x04\n" +
	"\bWorktree\x12\x1f\n" +
	"\vbranch_name\x18\x01 \x01(\tR\n" +
	"branchName\x12%\n" +
	"\x0edirectory_path\x18\x02 \x01(\tR\rdirectoryPath\x12-\n" +
	"\x10github_pr_number\x18\x03 \x01(\x03H\x00R\x0egithubPrNumber\x88\x01\x01\x12E\n" +
	"\x0elast_synced_at\x18\x05 \x01(\v2\x1a.google.protobuf.TimestampH\x01R\flastSyncedAt\x88\x01\x01\x121\n" +
	"\x14needs_reconciliation\x18\x06 \x01(\bR\x13needsReconciliation\x12\x14\n" +
	"\x05dirty\x18\a \x01(\bR\x05dirty\x12\x1a\n" +
	"\borphaned\x18\b \x01(\bR\borphaned\x12,\n" +
	"\x12local_only_commits\x18\t \x01(\x03R\x10localOnlyCommits\x12/\n" +
	"\x13reconciliation_note\x18\n" +
	" \x01(\tR\x12reconciliationNote\x12)\n" +
	"\x10branch_disparity\x18\v \x01(\bR\x0fbranchDisparity\x12,\n" +
	"\x12checked_out_branch\x18\f \x01(\tR\x10checkedOutBranchB\x13\n" +
	"\x11_github_pr_numberB\x11\n" +
	"\x0f_last_synced_atJ\x04\b\x04\x10\x05R\n" +
	"created_by\"\x0f\n" +
	"\rHealthRequest\"e\n" +
	"\x0eHealthResponse\x12\x18\n" +
	"\aversion\x18\x01 \x01(\tR\aversion\x129\n" +
	"\n" +
	"started_at\x18\x02 \x01(\v2\x1a.google.protobuf.TimestampR\tstartedAt\"6\n" +
	"\x15InitRepositoryRequest\x12\x1d\n" +
	"\n" +
	"local_path\x18\x01 \x01(\tR\tlocalPath\"\xbb\x01\n" +
	"\x16InitRepositoryResponse\x129\n" +
	"\n" +
	"repository\x18\x01 \x01(\v2\x19.lumberjack.v1.RepositoryR\n" +
	"repository\x12-\n" +
	"\x10adopted_branches\x18\x02 \x03(\tB\x02\x18\x01R\x0fadoptedBranches\x127\n" +
	"\aadopted\x18\x03 \x03(\v2\x1d.lumberjack.v1.WorktreeChangeR\aadopted\"\x19\n" +
	"\x17ListRepositoriesRequest\"Y\n" +
	"\x18ListRepositoriesResponse\x12=\n" +
	"\frepositories\x18\x01 \x03(\v2\x19.lumberjack.v1.RepositoryR\frepositories\"6\n" +
	"\x14GetRepositoryRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\"R\n" +
	"\x15GetRepositoryResponse\x129\n" +
	"\n" +
	"repository\x18\x01 \x01(\v2\x19.lumberjack.v1.RepositoryR\n" +
	"repository\"G\n" +
	"\x0fSetLoginRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\x12\x14\n" +
	"\x05login\x18\x02 \x01(\tR\x05login\"M\n" +
	"\x10SetLoginResponse\x129\n" +
	"\n" +
	"repository\x18\x01 \x01(\v2\x19.lumberjack.v1.RepositoryR\n" +
	"repository\"3\n" +
	"\x11ListLoginsRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\"F\n" +
	"\x12ListLoginsResponse\x12\x16\n" +
	"\x06logins\x18\x01 \x03(\tR\x06logins\x12\x18\n" +
	"\acurrent\x18\x02 \x01(\tR\acurrent\"<\n" +
	"\x16GetSetupConsentRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository:\x02\x18\x01\"Z\n" +
	"\x17GetSetupConsentResponse\x12\x18\n" +
	"\apending\x18\x01 \x01(\bR\apending\x12!\n" +
	"\frun_commands\x18\x02 \x03(\tR\vrunCommands:\x02\x18\x01\"T\n" +
	"\x16SetSetupConsentRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\x12\x1a\n" +
	"\bchecksum\x18\x02 \x01(\tR\bchecksum\"p\n" +
	"\x17SetSetupConsentResponse\x129\n" +
	"\n" +
	"repository\x18\x01 \x01(\v2\x19.lumberjack.v1.RepositoryR\n" +
	"repository\x12\x1a\n" +
	"\baccepted\x18\x02 \x01(\bR\baccepted\"T\n" +
	"\x16TrustSetupStepsRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\x12\x1a\n" +
	"\bchecksum\x18\x02 \x01(\tR\bchecksum\"T\n" +
	"\x17TrustSetupStepsResponse\x129\n" +
	"\n" +
	"repository\x18\x01 \x01(\v2\x19.lumberjack.v1.RepositoryR\n" +
	"repository\"6\n" +
	"\x14ListWorktreesRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\"N\n" +
	"\x15ListWorktreesResponse\x125\n" +
	"\tworktrees\x18\x01 \x03(\v2\x17.lumberjack.v1.WorktreeR\tworktrees\"L\n" +
	"\x12AddWorktreeRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\x12\x16\n" +
	"\x06branch\x18\x02 \x01(\tR\x06branch\"\x9c\x01\n" +
	"\x13AddWorktreeResponse\x12%\n" +
	"\x0edirectory_path\x18\x01 \x01(\tR\rdirectoryPath\x12\x16\n" +
	"\x06branch\x18\x02 \x01(\tR\x06branch\x12%\n" +
	"\x0ebranch_created\x18\x03 \x01(\bR\rbranchCreated\x12\x1f\n" +
	"\vsetup_error\x18\x04 \x01(\tR\n" +
	"setupError\"i\n" +
	"\x15DeleteWorktreeRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\x12\x1a\n" +
	"\bworktree\x18\x02 \x01(\tR\bworktree\x12\x14\n" +
	"\x05force\x18\x03 \x01(\bR\x05force\"\xa9\x01\n" +
	"\x16DeleteWorktreeResponse\x12\x18\n" +
	"\adeleted\x18\x01 \x01(\bR\adeleted\x123\n" +
	"\x15requires_confirmation\x18\x02 \x01(\bR\x14requiresConfirmation\x12&\n" +
	"\x0fcommits_at_risk\x18\x03 \x01(\x03R\rcommitsAtRisk\x12\x18\n" +
	"\amessage\x18\x04 \x01(\tR\amessage\"9\n" +
	"\x17DeleteRepositoryRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\"a\n" +
	"\x18DeleteRepositoryResponse\x12+\n" +
	"\x11worktrees_removed\x18\x01 \x01(\x03R\x10worktreesRemoved\x12\x18\n" +
	"\amessage\x18\x02 \x01(\tR\amessage\"-\n" +
	"\vSyncRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\"\xd3\x01\n" +
	"\fSyncResponse\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\x12\x18\n" +
	"\amessage\x18\x02 \x01(\tR\amessage\x12\x1c\n" +
	"\tcompleted\x18\x03 \x01(\bR\tcompleted\x124\n" +
	"\asummary\x18\x04 \x01(\v2\x1a.lumberjack.v1.SyncSummaryR\asummary\x125\n" +
	"\x06change\x18\x05 \x01(\v2\x1d.lumberjack.v1.WorktreeChangeR\x06change\"\xbf\x01\n" +
	"\vSyncSummary\x121\n" +
	"\x06status\x18\x01 \x01(\x0e2\x19.lumberjack.v1.SyncStatusR\x06status\x12+\n" +
	"\x11worktrees_created\x18\x02 \x01(\x03R\x10worktreesCreated\x12+\n" +
	"\x11worktrees_removed\x18\x03 \x01(\x03R\x10worktreesRemoved\x12\x19\n" +
	"\x05error\x18\x04 \x01(\tH\x00R\x05error\x88\x01\x01B\b\n" +
	"\x06_error\"l\n" +
	"\fLockDecision\x12#\n" +
	"\rworktree_path\x18\x01 \x01(\tR\fworktreePath\x127\n" +
	"\bstrategy\x18\x02 \x01(\x0e2\x1b.lumberjack.v1.LockStrategyR\bstrategy\"\xe8\x01\n" +
	"\vTidyRequest\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\x12\x17\n" +
	"\adry_run\x18\x02 \x01(\bR\x06dryRun\x12\x1a\n" +
	"\bworktree\x18\x03 \x01(\tR\bworktree\x12@\n" +
	"\rlock_strategy\x18\x04 \x01(\x0e2\x1b.lumberjack.v1.LockStrategyR\flockStrategy\x12B\n" +
	"\x0elock_decisions\x18\x05 \x03(\v2\x1b.lumberjack.v1.LockDecisionR\rlockDecisions\"\xcb\x01\n" +
	"\bTidyMove\x12\x1e\n" +
	"\n" +
	"repository\x18\x01 \x01(\tR\n" +
	"repository\x12\x16\n" +
	"\x06branch\x18\x02 \x01(\tR\x06branch\x12\x12\n" +
	"\x04from\x18\x03 \x01(\tR\x04from\x12\x0e\n" +
	"\x02to\x18\x04 \x01(\tR\x02to\x12\x14\n" +
	"\x05moved\x18\x05 \x01(\bR\x05moved\x12\x14\n" +
	"\x05error\x18\x06 \x01(\tR\x05error\x12\x16\n" +
	"\x06locked\x18\a \x01(\bR\x06locked\x12\x1f\n" +
	"\vlock_reason\x18\b \x01(\tR\n" +
	"lockReason\"=\n" +
	"\fTidyResponse\x12-\n" +
	"\x05moves\x18\x01 \x03(\v2\x17.lumberjack.v1.TidyMoveR\x05moves\"\x0e\n" +
	"\fWatchRequest\"\xa4\x02\n" +
	"\rWatchResponse\x124\n" +
	"\x04type\x18\x01 \x01(\x0e2 .lumberjack.v1.WatchResponseTypeR\x04type\x129\n" +
	"\n" +
	"repository\x18\x02 \x01(\v2\x19.lumberjack.v1.RepositoryR\n" +
	"repository\x125\n" +
	"\tworktrees\x18\x03 \x03(\v2\x17.lumberjack.v1.WorktreeR\tworktrees\x125\n" +
	"\x06change\x18\x04 \x01(\v2\x1d.lumberjack.v1.WorktreeChangeR\x06change\x124\n" +
	"\asummary\x18\x05 \x01(\v2\x1a.lumberjack.v1.SyncSummaryR\asummary*T\n" +
	"\n" +
	"SyncStatus\x12\x1b\n" +
	"\x17SYNC_STATUS_UNSPECIFIED\x10\x00\x12\x12\n" +
	"\x0eSYNC_STATUS_OK\x10\x01\x12\x15\n" +
	"\x11SYNC_STATUS_ERROR\x10\x02*\xc7\x01\n" +
	"\x0eWorktreeAction\x12\x1f\n" +
	"\x1bWORKTREE_ACTION_UNSPECIFIED\x10\x00\x12\x1f\n" +
	"\x1bWORKTREE_ACTION_CHECKED_OUT\x10\x01\x12\x1b\n" +
	"\x17WORKTREE_ACTION_ADOPTED\x10\x02\x12\x1b\n" +
	"\x17WORKTREE_ACTION_UPDATED\x10\x03\x12\x1b\n" +
	"\x17WORKTREE_ACTION_DELETED\x10\x04\x12\x1c\n" +
	"\x18WORKTREE_ACTION_RETAINED\x10\x05*\x92\x01\n" +
	"\fLockStrategy\x12\x1d\n" +
	"\x19LOCK_STRATEGY_UNSPECIFIED\x10\x00\x12\x16\n" +
	"\x12LOCK_STRATEGY_SKIP\x10\x01\x12\x18\n" +
	"\x14LOCK_STRATEGY_UNLOCK\x10\x02\x12\x18\n" +
	"\x14LOCK_STRATEGY_DELETE\x10\x03\x12\x17\n" +
	"\x13LOCK_STRATEGY_ABORT\x10\x04*\xd1\x01\n" +
	"\x11WatchResponseType\x12#\n" +
	"\x1fWATCH_RESPONSE_TYPE_UNSPECIFIED\x10\x00\x12 \n" +
	"\x1cWATCH_RESPONSE_TYPE_SNAPSHOT\x10\x01\x12(\n" +
	"$WATCH_RESPONSE_TYPE_WORKTREE_CHANGED\x10\x02\x12$\n" +
	" WATCH_RESPONSE_TYPE_SYNC_STARTED\x10\x03\x12%\n" +
	"!WATCH_RESPONSE_TYPE_SYNC_FINISHED\x10\x042\x85\v\n" +
	"\x11LumberjackService\x12E\n" +
	"\x06Health\x12\x1c.lumberjack.v1.HealthRequest\x1a\x1d.lumberjack.v1.HealthResponse\x12]\n" +
	"\x0eInitRepository\x12$.lumberjack.v1.InitRepositoryRequest\x1a%.lumberjack.v1.InitRepositoryResponse\x12c\n" +
	"\x10ListRepositories\x12&.lumberjack.v1.ListRepositoriesRequest\x1a'.lumberjack.v1.ListRepositoriesResponse\x12Z\n" +
	"\rGetRepository\x12#.lumberjack.v1.GetRepositoryRequest\x1a$.lumberjack.v1.GetRepositoryResponse\x12K\n" +
	"\bSetLogin\x12\x1e.lumberjack.v1.SetLoginRequest\x1a\x1f.lumberjack.v1.SetLoginResponse\x12Q\n" +
	"\n" +
	"ListLogins\x12 .lumberjack.v1.ListLoginsRequest\x1a!.lumberjack.v1.ListLoginsResponse\x12Z\n" +
	"\rListWorktrees\x12#.lumberjack.v1.ListWorktreesRequest\x1a$.lumberjack.v1.ListWorktreesResponse\x12T\n" +
	"\vAddWorktree\x12!.lumberjack.v1.AddWorktreeRequest\x1a\".lumberjack.v1.AddWorktreeResponse\x12]\n" +
	"\x0eDeleteWorktree\x12$.lumberjack.v1.DeleteWorktreeRequest\x1a%.lumberjack.v1.DeleteWorktreeResponse\x12c\n" +
	"\x10DeleteRepository\x12&.lumberjack.v1.DeleteRepositoryRequest\x1a'.lumberjack.v1.DeleteRepositoryResponse\x12A\n" +
	"\x04Sync\x12\x1a.lumberjack.v1.SyncRequest\x1a\x1b.lumberjack.v1.SyncResponse0\x01\x12?\n" +
	"\x04Tidy\x12\x1a.lumberjack.v1.TidyRequest\x1a\x1b.lumberjack.v1.TidyResponse\x12D\n" +
	"\x05Watch\x12\x1b.lumberjack.v1.WatchRequest\x1a\x1c.lumberjack.v1.WatchResponse0\x01\x12e\n" +
	"\x0fGetSetupConsent\x12%.lumberjack.v1.GetSetupConsentRequest\x1a&.lumberjack.v1.GetSetupConsentResponse\"\x03\x88\x02\x01\x12`\n" +
	"\x0fSetSetupConsent\x12%.lumberjack.v1.SetSetupConsentRequest\x1a&.lumberjack.v1.SetSetupConsentResponse\x12`\n" +
	"\x0fTrustSetupSteps\x12%.lumberjack.v1.TrustSetupStepsRequest\x1a&.lumberjack.v1.TrustSetupStepsResponseBIZGgithub.com/ceilingfish/lumberjack/pkg/client/lumberjack/v1;lumberjackv1b\x06proto3"

var (
	file_lumberjack_v1_lumberjack_proto_rawDescOnce sync.Once
	file_lumberjack_v1_lumberjack_proto_rawDescData []byte
)

func file_lumberjack_v1_lumberjack_proto_rawDescGZIP() []byte {
	file_lumberjack_v1_lumberjack_proto_rawDescOnce.Do(func() {
		file_lumberjack_v1_lumberjack_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_lumberjack_v1_lumberjack_proto_rawDesc), len(file_lumberjack_v1_lumberjack_proto_rawDesc)))
	})
	return file_lumberjack_v1_lumberjack_proto_rawDescData
}

var file_lumberjack_v1_lumberjack_proto_enumTypes = make([]protoimpl.EnumInfo, 4)
var file_lumberjack_v1_lumberjack_proto_msgTypes = make([]protoimpl.MessageInfo, 39)
var file_lumberjack_v1_lumberjack_proto_goTypes = []any{
	(SyncStatus)(0),                  // 0: lumberjack.v1.SyncStatus
	(WorktreeAction)(0),              // 1: lumberjack.v1.WorktreeAction
	(LockStrategy)(0),                // 2: lumberjack.v1.LockStrategy
	(WatchResponseType)(0),           // 3: lumberjack.v1.WatchResponseType
	(*WorktreeChange)(nil),           // 4: lumberjack.v1.WorktreeChange
	(*Repository)(nil),               // 5: lumberjack.v1.Repository
	(*SetupSteps)(nil),               // 6: lumberjack.v1.SetupSteps
	(*Worktree)(nil),                 // 7: lumberjack.v1.Worktree
	(*HealthRequest)(nil),            // 8: lumberjack.v1.HealthRequest
	(*HealthResponse)(nil),           // 9: lumberjack.v1.HealthResponse
	(*InitRepositoryRequest)(nil),    // 10: lumberjack.v1.InitRepositoryRequest
	(*InitRepositoryResponse)(nil),   // 11: lumberjack.v1.InitRepositoryResponse
	(*ListRepositoriesRequest)(nil),  // 12: lumberjack.v1.ListRepositoriesRequest
	(*ListRepositoriesResponse)(nil), // 13: lumberjack.v1.ListRepositoriesResponse
	(*GetRepositoryRequest)(nil),     // 14: lumberjack.v1.GetRepositoryRequest
	(*GetRepositoryResponse)(nil),    // 15: lumberjack.v1.GetRepositoryResponse
	(*SetLoginRequest)(nil),          // 16: lumberjack.v1.SetLoginRequest
	(*SetLoginResponse)(nil),         // 17: lumberjack.v1.SetLoginResponse
	(*ListLoginsRequest)(nil),        // 18: lumberjack.v1.ListLoginsRequest
	(*ListLoginsResponse)(nil),       // 19: lumberjack.v1.ListLoginsResponse
	(*GetSetupConsentRequest)(nil),   // 20: lumberjack.v1.GetSetupConsentRequest
	(*GetSetupConsentResponse)(nil),  // 21: lumberjack.v1.GetSetupConsentResponse
	(*SetSetupConsentRequest)(nil),   // 22: lumberjack.v1.SetSetupConsentRequest
	(*SetSetupConsentResponse)(nil),  // 23: lumberjack.v1.SetSetupConsentResponse
	(*TrustSetupStepsRequest)(nil),   // 24: lumberjack.v1.TrustSetupStepsRequest
	(*TrustSetupStepsResponse)(nil),  // 25: lumberjack.v1.TrustSetupStepsResponse
	(*ListWorktreesRequest)(nil),     // 26: lumberjack.v1.ListWorktreesRequest
	(*ListWorktreesResponse)(nil),    // 27: lumberjack.v1.ListWorktreesResponse
	(*AddWorktreeRequest)(nil),       // 28: lumberjack.v1.AddWorktreeRequest
	(*AddWorktreeResponse)(nil),      // 29: lumberjack.v1.AddWorktreeResponse
	(*DeleteWorktreeRequest)(nil),    // 30: lumberjack.v1.DeleteWorktreeRequest
	(*DeleteWorktreeResponse)(nil),   // 31: lumberjack.v1.DeleteWorktreeResponse
	(*DeleteRepositoryRequest)(nil),  // 32: lumberjack.v1.DeleteRepositoryRequest
	(*DeleteRepositoryResponse)(nil), // 33: lumberjack.v1.DeleteRepositoryResponse
	(*SyncRequest)(nil),              // 34: lumberjack.v1.SyncRequest
	(*SyncResponse)(nil),             // 35: lumberjack.v1.SyncResponse
	(*SyncSummary)(nil),              // 36: lumberjack.v1.SyncSummary
	(*LockDecision)(nil),             // 37: lumberjack.v1.LockDecision
	(*TidyRequest)(nil),              // 38: lumberjack.v1.TidyRequest
	(*TidyMove)(nil),                 // 39: lumberjack.v1.TidyMove
	(*TidyResponse)(nil),             // 40: lumberjack.v1.TidyResponse
	(*WatchRequest)(nil),             // 41: lumberjack.v1.WatchRequest
	(*WatchResponse)(nil),            // 42: lumberjack.v1.WatchResponse
	(*timestamppb.Timestamp)(nil),    // 43: google.protobuf.Timestamp
}
var file_lumberjack_v1_lumberjack_proto_depIdxs = []int32{
	1,  // 0: lumberjack.v1.WorktreeChange.action:type_name -> lumberjack.v1.WorktreeAction
	43, // 1: lumberjack.v1.WorktreeChange.last_synced_at:type_name -> google.protobuf.Timestamp
	43, // 2: lumberjack.v1.Repository.last_synced_at:type_name -> google.protobuf.Timestamp
	0,  // 3: lumberjack.v1.Repository.last_sync_status:type_name -> lumberjack.v1.SyncStatus
	43, // 4: lumberjack.v1.Repository.created_at:type_name -> google.protobuf.Timestamp
	6,  // 5: lumberjack.v1.Repository.setup_steps:type_name -> lumberjack.v1.SetupSteps
	43, // 6: lumberjack.v1.Worktree.last_synced_at:type_name -> google.protobuf.Timestamp
	43, // 7: lumberjack.v1.HealthResponse.started_at:type_name -> google.protobuf.Timestamp
	5,  // 8: lumberjack.v1.InitRepositoryResponse.repository:type_name -> lumberjack.v1.Repository
	4,  // 9: lumberjack.v1.InitRepositoryResponse.adopted:type_name -> lumberjack.v1.WorktreeChange
	5,  // 10: lumberjack.v1.ListRepositoriesResponse.repositories:type_name -> lumberjack.v1.Repository
	5,  // 11: lumberjack.v1.GetRepositoryResponse.repository:type_name -> lumberjack.v1.Repository
	5,  // 12: lumberjack.v1.SetLoginResponse.repository:type_name -> lumberjack.v1.Repository
	5,  // 13: lumberjack.v1.SetSetupConsentResponse.repository:type_name -> lumberjack.v1.Repository
	5,  // 14: lumberjack.v1.TrustSetupStepsResponse.repository:type_name -> lumberjack.v1.Repository
	7,  // 15: lumberjack.v1.ListWorktreesResponse.worktrees:type_name -> lumberjack.v1.Worktree
	36, // 16: lumberjack.v1.SyncResponse.summary:type_name -> lumberjack.v1.SyncSummary
	4,  // 17: lumberjack.v1.SyncResponse.change:type_name -> lumberjack.v1.WorktreeChange
	0,  // 18: lumberjack.v1.SyncSummary.status:type_name -> lumberjack.v1.SyncStatus
	2,  // 19: lumberjack.v1.LockDecision.strategy:type_name -> lumberjack.v1.LockStrategy
	2,  // 20: lumberjack.v1.TidyRequest.lock_strategy:type_name -> lumberjack.v1.LockStrategy
	37, // 21: lumberjack.v1.TidyRequest.lock_decisions:type_name -> lumberjack.v1.LockDecision
	39, // 22: lumberjack.v1.TidyResponse.moves:type_name -> lumberjack.v1.TidyMove
	3,  // 23: lumberjack.v1.WatchResponse.type:type_name -> lumberjack.v1.WatchResponseType
	5,  // 24: lumberjack.v1.WatchResponse.repository:type_name -> lumberjack.v1.Repository
	7,  // 25: lumberjack.v1.WatchResponse.worktrees:type_name -> lumberjack.v1.Worktree
	4,  // 26: lumberjack.v1.WatchResponse.change:type_name -> lumberjack.v1.WorktreeChange
	36, // 27: lumberjack.v1.WatchResponse.summary:type_name -> lumberjack.v1.SyncSummary
	8,  // 28: lumberjack.v1.LumberjackService.Health:input_type -> lumberjack.v1.HealthRequest
	10, // 29: lumberjack.v1.LumberjackService.InitRepository:input_type -> lumberjack.v1.InitRepositoryRequest
	12, // 30: lumberjack.v1.LumberjackService.ListRepositories:input_type -> lumberjack.v1.ListRepositoriesRequest
	14, // 31: lumberjack.v1.LumberjackService.GetRepository:input_type -> lumberjack.v1.GetRepositoryRequest
	16, // 32: lumberjack.v1.LumberjackService.SetLogin:input_type -> lumberjack.v1.SetLoginRequest
	18, // 33: lumberjack.v1.LumberjackService.ListLogins:input_type -> lumberjack.v1.ListLoginsRequest
	26, // 34: lumberjack.v1.LumberjackService.ListWorktrees:input_type -> lumberjack.v1.ListWorktreesRequest
	28, // 35: lumberjack.v1.LumberjackService.AddWorktree:input_type -> lumberjack.v1.AddWorktreeRequest
	30, // 36: lumberjack.v1.LumberjackService.DeleteWorktree:input_type -> lumberjack.v1.DeleteWorktreeRequest
	32, // 37: lumberjack.v1.LumberjackService.DeleteRepository:input_type -> lumberjack.v1.DeleteRepositoryRequest
	34, // 38: lumberjack.v1.LumberjackService.Sync:input_type -> lumberjack.v1.SyncRequest
	38, // 39: lumberjack.v1.LumberjackService.Tidy:input_type -> lumberjack.v1.TidyRequest
	41, // 40: lumberjack.v1.LumberjackService.Watch:input_type -> lumberjack.v1.WatchRequest
	20, // 41: lumberjack.v1.LumberjackService.GetSetupConsent:input_type -> lumberjack.v1.GetSetupConsentRequest
	22, // 42: lumberjack.v1.LumberjackService.SetSetupConsent:input_type -> lumberjack.v1.SetSetupConsentRequest
	24, // 43: lumberjack.v1.LumberjackService.TrustSetupSteps:input_type -> lumberjack.v1.TrustSetupStepsRequest
	9,  // 44: lumberjack.v1.LumberjackService.Health:output_type -> lumberjack.v1.HealthResponse
	11, // 45: lumberjack.v1.LumberjackService.InitRepository:output_type -> lumberjack.v1.InitRepositoryResponse
	13, // 46: lumberjack.v1.LumberjackService.ListRepositories:output_type -> lumberjack.v1.ListRepositoriesResponse
	15, // 47: lumberjack.v1.LumberjackService.GetRepository:output_type -> lumberjack.v1.GetRepositoryResponse
	17, // 48: lumberjack.v1.LumberjackService.SetLogin:output_type -> lumberjack.v1.SetLoginResponse
	19, // 49: lumberjack.v1.LumberjackService.ListLogins:output_type -> lumberjack.v1.ListLoginsResponse
	27, // 50: lumberjack.v1.LumberjackService.ListWorktrees:output_type -> lumberjack.v1.ListWorktreesResponse
	29, // 51: lumberjack.v1.LumberjackService.AddWorktree:output_type -> lumberjack.v1.AddWorktreeResponse
	31, // 52: lumberjack.v1.LumberjackService.DeleteWorktree:output_type -> lumberjack.v1.DeleteWorktreeResponse
	33, // 53: lumberjack.v1.LumberjackService.DeleteRepository:output_type -> lumberjack.v1.DeleteRepositoryResponse
	35, // 54: lumberjack.v1.LumberjackService.Sync:output_type -> lumberjack.v1.SyncResponse
	40, // 55: lumberjack.v1.LumberjackService.Tidy:output_type -> lumberjack.v1.TidyResponse
	42, // 56: lumberjack.v1.LumberjackService.Watch:output_type -> lumberjack.v1.WatchResponse
	21, // 57: lumberjack.v1.LumberjackService.GetSetupConsent:output_type -> lumberjack.v1.GetSetupConsentResponse
	23, // 58: lumberjack.v1.LumberjackService.SetSetupConsent:output_type -> lumberjack.v1.SetSetupConsentResponse
	25, // 59: lumberjack.v1.LumberjackService.TrustSetupSteps:output_type -> lumberjack.v1.TrustSetupStepsResponse
	44, // [44:60] is the sub-list for method output_type
	28, // [28:44] is the sub-list for method input_type
	28, // [28:28] is the sub-list for extension type_name
	28, // [28:28] is the sub-list for extension extendee
	0,  // [0:28] is the sub-list for field type_name
}

func init() { file_lumberjack_v1_lumberjack_proto_init() }
func file_lumberjack_v1_lumberjack_proto_init() {
	if File_lumberjack_v1_lumberjack_proto != nil {
		return
	}
	file_lumberjack_v1_lumberjack_proto_msgTypes[0].OneofWrappers = []any{}
	file_lumberjack_v1_lumberjack_proto_msgTypes[1].OneofWrappers = []any{}
	file_lumberjack_v1_lumberjack_proto_msgTypes[3].OneofWrappers = []any{}
	file_lumberjack_v1_lumberjack_proto_msgTypes[32].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_lumberjack_v1_lumberjack_proto_rawDesc), len(file_lumberjack_v1_lumberjack_proto_rawDesc)),
			NumEnums:      4,
			NumMessages:   39,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_lumberjack_v1_lumberjack_proto_goTypes,
		DependencyIndexes: file_lumberjack_v1_lumberjack_proto_depIdxs,
		EnumInfos:         file_lumberjack_v1_lumberjack_proto_enumTypes,
		MessageInfos:      file_lumberjack_v1_lumberjack_proto_msgTypes,
	}.Build()
	File_lumberjack_v1_lumberjack_proto = out.File
	file_lumberjack_v1_lumberjack_proto_goTypes = nil
	file_lumberjack_v1_lumberjack_proto_depIdxs = nil
}
